...
st.image("Total_Sales_Revenue_Predition/luckyfloorsbc_LOGO.JPG",width=600, caption="Could you guess the Total Sales?")
st.subheader(" What could be the  sales income in next days, weeks and months?")

st.image("Total_Sales_Revenue_Predition/flooring.jpg",width=200)
...
if st.checkbox("Month by month prediction"):
    test_=pd.read_csv("Total_Sales_Revenue_Predition/ARIMA_test_monthly_all.csv").set_index("Unnamed: 0") 
...
if st.checkbox("Week by week prediction"):
    test_=pd.read_csv("Total_Sales_Revenue_Predition/ARIMA_test_weekly_all.csv").set_index("Unnamed: 0") 
    test_.index = pd.to_datetime(test_.index)
...
if st.checkbox("Day by day prediction"):
    test_=pd.read_csv("Total_Sales_Revenue_Predition/ARIMA_test_daily_all.csv").set_index("Unnamed: 0") 
    test_.index = pd.to_datetime(test_.index)
